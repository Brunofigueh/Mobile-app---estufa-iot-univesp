...
const DEFAULT_API_URL = 'http://192.168.1.25:8000'; // IP Padrão 
...
export const ApiService = {
  ...
  getHistoricoLeituras: async (limit: number = 20): Promise<Leitura[]> => {
    ...
    const response = await apiClient.get<Leitura[]>(`/api/leituras/historico?limit=${limit}`);
    return response.data;
  },
  ...
  getAlertasAtivos: async (): Promise<Alerta[]> => {
    ...
    const response = await apiClient.get<Alerta[]>('/api/alertas/ativos');
    return response.data;
  },
